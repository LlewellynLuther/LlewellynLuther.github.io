...
var addDiv = document.getElementById('add');				//+分数	（每步加的分数）
...
// 生成矩阵
function initGame() {
  game = Array(size * size).fill(null); 					// 4 x 4 矩阵，用null填充数组（清空）
  initBestScore();
}
...
function drawBackground() {
  var tileContainer = document.getElementById('tile-container');	//主体区
  ...
  for (var i = 0; i < game.length; i++) {
	...
	tileContainer.appendChild(tileDiv);								//加入子集
  }
}
...
function newGameStart() {
	document.getElementById('tile-container').innerHTML = '';			//主体区清空
